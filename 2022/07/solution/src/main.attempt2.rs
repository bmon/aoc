...
fn lv1(reader: &mut BufReader<File>) -> Result<i32, CalcError> {
    let mut dirs: HashMap<String, &mut ChallengeDir> = HashMap::new();
    let mut cwd = &mut ChallengeDir::new("/".to_owned());
    dirs.insert("/".to_owned(), cwd);
    ...
    for line in reader.lines() {
        ...
        match ln {
            ...
            _ if ln.starts_with("dir") => {
                ...
                let newdir = cwd.add_dir(dirname.to_owned());
                let res = dirs.insert(dirname.into(), newdir);
                match res {
                    Some(_) => {
...
impl ChallengeDir {
    ...
    fn add_dir(&mut self, name: String) -> &mut ChallengeDir {
        self.dirs.push(ChallengeDir::new(name));
